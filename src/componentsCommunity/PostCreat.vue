<template>
  <div class="main-container">
    ...
    <div class="back-button-container">
      <el-button @click="confirmGoBack" icon="el-icon-arrow-left">返回{{ communityName }}社区</el-button>
    </div>
...
export default {
  ...
  setup() {
    ...
    const publish = () => {
      const postData = {
        userId: getUserId(), // 假设getUserId是一个方法，用于获取用户ID
        communityId: communityId.value, // 使用社区ID
    ...
    const getUserId = () => {
      // 这里需要实现获取用户ID的逻辑，可能是从路由参数、localStorage或者Vuex状态管理中获取
      // 假设用户ID存储在sessionStorage中
      return sessionStorage.getItem('userId'); // 如果用户未登录，会返回null
    };
