<template>
  ...
  <div class="outer-layout">
    ...
    <!-- 悬浮侧边栏容器 -->
    <div
      class="floating-sidebar glowing-border"
    ...
    >
      ...
      <el-card class="sidebar-card neon-card">
        <div class="card-header">
          ...
          <transition name="fade-slide">
            <span v-show="isExpanded" class="gradient-text">推荐课程</span>
          </transition>
        </div>
        <div class="compact-list">
          <div
            v-for="(course, index) in recommendedCourses.slice(0, 4)"
            :key="course.id"
            class="compact-item hover-glow"
      ...
      <el-card class="sidebar-card neon-card" style="margin-top: 12px">
        <div class="card-header">
          ...
          <transition name="fade-slide">
            <span v-show="isExpanded" class="gradient-text">推荐赛事</span>
          </transition>
        </div>
        <div class="compact-list">
          <div
            v-for="(comp, index) in recommendedCompetitions.slice(0, 4)"
            :key="comp.id"
            class="compact-item hover-glow"
            @click.stop="viewCompetition(comp.id)"
          >
            <div class="item-index">{{ index + 1 }}</div>
            <div class="item-content">
        ...
        <el-aside width="25%">
          <el-card class="profile-container">
              ...
              <el-space wrap>
                ...
                >
                  <template #reference>
                    <el-avatar
                      :size="80"
                      :src="getImgUrl(userInfo.userProfilePicture)"
            ...
            <div class="bio-section">
              ...
              <div
                v-show="userInfo.knowledgeNetwork?.length"
                ref="knowledgeChart"
      ...
      <el-row :gutter="24">
        ...
        <el-col :span="1">
          <el-upload
            ...
            action="http://localhost:10086/uis/v1/upload"
            :show-file-list="false"
            :on-success="handleAvatarSuccess"
            :before-upload="beforeAvatarUpload"
          >
            <img
              v-if="form.userProfilePicture"
              :src="getImgUrl(form.userProfilePicture)"
              class="avatar"
            />
  ...
  >
    <el-tabs type="border-card">
      <el-tab-pane
        v-for="(category, index) in Object.values(knowledgeItems)"
        :key="index"
      >
        <template #label>
